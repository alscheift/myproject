<section>
    ...
    <form method="post" action="{{ route('book.update', $book->id) }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            <x-text-input id="description" name="description" type="text" class="mt-1 block w-full" :value="old('description', $book->description)" required autofocus autocomplete="name" />
        ...
        <div>
            ...
            <select id="category_id" name="category_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                ...
                @foreach($categories as $category)
                    <option value="{{$category->id}}" @selected($book->category->id)>{{$category->name}}</option>
                @endforeach
            </select>
        </div>
